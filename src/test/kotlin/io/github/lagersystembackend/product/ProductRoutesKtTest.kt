...
class ProductRoutesKtTest {
    ...
    fun `get Products should respond with List of NetworkProducts`() = testApplication {
        createEnvironment()
        val products = listOf(
            Product(UUID.randomUUID().toString(), "Space 1",  "Description 1",  null, null, emptyMap(), LocalDateTime.now(), LocalDateTime.now(), false),
            Product(UUID.randomUUID().toString(), "Space 2", "Description 2", null, null, mapOf("someKey" to Attribute.NumberAttribute(123f)), LocalDateTime.now(), LocalDateTime.now(), false)
        )
    ...
    fun `delete Product should delete Product`() = testApplication {
        createEnvironment()
        val product1 = Product(UUID.randomUUID().toString(), "Product 1", "Description 1", null, null, emptyMap(), LocalDateTime.now(), LocalDateTime.now(), false)
    ...
    fun `post Product should create Product`() = testApplication {
        ...
        val addProductNetworkRequest =
            AddProductNetworkRequest("Product 1", "Description 1", null, null, false)
        addProductNetworkRequest.run {
            val product = Product(id, name, description, null, null, emptyMap(), createTime, createTime, false)
            every { mockProductRepository.createProduct(name, description, null, null, false) } returns product
        }
    ...
    fun `post Product should create Product when price is null`() = testApplication {
        ...
        val id = UUID.randomUUID().toString()
        val addProductNetworkRequest =
            AddProductNetworkRequest("Space 1", "Description 1", null, null, false)
        addProductNetworkRequest.run {
            val product = Product(id, name, description, null, null, emptyMap(), createTime, createTime, false)
            every { mockProductRepository.createProduct(name, description, null, null, false) } returns product
        }
